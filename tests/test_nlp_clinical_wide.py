...
from cumulus_library_pcx.stage.nlp_clinical_wide import prepare_resources


def test_prepare_writes_the_files_its_toml_lists(tmp_path):
    paths = prepare_resources(output_dir=tmp_path)
...
def test_bad_deployment_suffix_writes_nothing(tmp_path):
    ...
    with pytest.raises(ValueError, match="deployment suffixes"):
        prepare_resources(["bad-suffix"], output_dir=tmp_path)
    with pytest.raises(ValueError, match="deployment suffixes"):
        prepare_resources([], output_dir=tmp_path)
