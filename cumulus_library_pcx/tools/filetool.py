...
def path_tests_data(test_file: Path | str = None) -> Path:
    """
    tests/data: schema.sql (every table the SQL tests and the synthetic generator use)
    """
    if not test_file:
        return path_tests() / 'data'
    return path_tests() / 'data' / test_file

def path_tests_data_warn(test_file: Path | str = None) -> Path:
    """
    tests/data/warn: hand-written fixtures, one CSV per table, each subject seeded with anomalies
    """
    return path_tests_data('warn') / test_file if test_file else path_tests_data('warn')

def path_tests_data_synthetic(test_file: Path | str = None) -> Path:
    """
    tests/data/synthetic: generated by `make-pcx test-synthetic`, regenerate rather than edit
    """
