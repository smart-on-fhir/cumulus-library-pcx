...
Options
    ...
    --include-inputs
                    also write the upstream tables as plain CSV to OUTPUT_DIR/input, in the
                    tests/data fixture style, to load with schema.sql.
    --seed S        same seed, same files. Patient N has the same latent truth at any --noise.

Requires duckdb and numpy (pip install -e ".[test]").
...
def write_plain_csv(path: Path, columns: list[str], rows: list[dict]) -> None:
    """
    Upstream tables in the same plain style as the tests/data fixtures (NULL is empty).
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument('--include-inputs', action='store_true',
                        help=f'also keep the upstream tables (schema.sql) as plain CSV in OUTPUT_DIR/{INPUT_DIRNAME}')
    ...
    if output_dir == filetool.path_tests_data().resolve():
        parser.error('tests/data holds the hand-written SQL test fixtures, choose another directory')
    ...
    with tempfile.TemporaryDirectory() as scratch:
        ...
        if args.include_inputs:
            input_dir = output_dir / INPUT_DIRNAME
            input_dir.mkdir(exist_ok=True)
        con = build_database(tables, input_dir)
